...
uint16_t anchors[NUM_ANCHORS] = {0x6078, 0x603F, 0x6050, 0x6000};     // the network id of the anchors: change these to the network ids of your anchors.
int32_t heights[NUM_ANCHORS] = {730, 730, 730, 730};            // anchor z-coordinates in mm
...
int32_t anchors_x[NUM_ANCHORS] = {0, 600, 0, 0};              // anchor x-coorindates in mm
int32_t anchors_y[NUM_ANCHORS] = {0, 0, 600, 0};                  // anchor y-coordinates in mm
...
void loop() {
  ...
  pos_error_t error;
  int status1 = Pozyx.doPositioning(&position, POZYX_2_5D, 1000);
  int status2 = Pozyx.getPositionError(&error);
  if (status1 == status2 == POZYX_SUCCESS)
    printCoordVarCovar(&position, &error);
...
// function to print the coordinates, variances and covariances
void printCoordVarCovar(coordinates_t* coor, pos_error_t* error) {
  Serial.print(coor->x);
  Serial.print(" ");
  Serial.print(coor->y);
  Serial.print(" ");
  ...
  Serial.print(" ");
  Serial.print(error->xy);
  Serial.print(" ");
  Serial.print(error->xz);
  Serial.print(" ");
  Serial.print(error->yz);
  Serial.println();
...
void printCoordinatesProcessing(coordinates_t coor) {
  ...
  Serial.print(coor.z);
  Serial.print(",");
  //  Serial.print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX");
